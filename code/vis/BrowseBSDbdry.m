...
function plotFrame
	...
	for iInout = 1:nInout,
		...
		imagesc(ind2rgb(floor(1e3*mean(squeeze(1-inout{iFrame}),3)),gray(1e3)))
		axis image
		title(legendStr{iInout})

		ax(2*iInout+2) = subplot(2,nInout+1,nInout+2+iInout);
% 		conn = inout2conn(squeeze(inout{iFrame}),mknhood2d(1));
%         segEst = connectedComponents(conn>threshold,mknhood2d(1),50);
%         segEst = markerWatershed(conn,mknhood2d(1),segEst);
		segEst = watershed(imhmin(1-squeeze(inout{iFrame}),threshold,4),4);
